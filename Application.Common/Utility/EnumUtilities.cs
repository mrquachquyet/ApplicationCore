...
namespace Application.Common.Utility
{
    public static class EnumUtilities
    {
        ...
        public static IEnumerable<SelectListItem> GenerateSelectListItems<T>(GenerateEnumType generateType = GenerateEnumType.IntValueAndStringText,
            T selectedValue = default(T), SortEnumBy sortBy = SortEnumBy.Text) where T : struct
        {
            ...
            switch (sortBy)
            {
                case SortEnumBy.Text:
                    return selectList.OrderBy(item => item.Text);
                case SortEnumBy.Value:
                    return selectList.OrderBy(item => item.Value);
                default:
                    return selectList;
